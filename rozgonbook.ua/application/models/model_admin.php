...
class Model_Admin extends Model
{
    ...
    public function get_data()
    {
        ...
        $pdoDB = new DB;
        $pdo = $pdoDB->getDB();
        $sql_book = $pdo->prepare("SELECT B.id, B.name, B.year, B.img, B.book_delete, B.data_delet, B.look, B.click, 
                     GROUP_CONCAT( A.name_author SEPARATOR ', ') AS name_author 
                     FROM book B 
                     JOIN author_book C ON B.id = C.book_id 
                     JOIN author A ON C.author_id = A.id 
                     WHERE B.name LIKE :search OR
                     A.name_author LIKE :search OR B.year LIKE :search 
                     GROUP BY C.book_id");
        ...
        $array = array_chunk($result_book, 5);
        if ($page >= count($array)) {
            $page = 0;
        }
        if (empty($array[$page])) {
